...
from calcchain_core.common.errors import PublishError
from calcchain_core.models import SourceRef, SourceType, TargetRef
from calcchain_capabilities import AuthServiceProtocol, CapabilityOwner, PluginRefMetadata, TargetContext
...
class _RuntimeTargetAdapter:
    def __init__(self, adapter, *, capability_id: str, owner: CapabilityOwner | None, auth: AuthServiceProtocol | None) -> None:
    ...
    def write_file(self, target: TargetRef, relative_path: str, data: bytes) -> PublishedRef:
        ...
        published = self._call_plugin('write_file', target, relative_path, data, self._context())
    ...
    def ensure_root(self, target: TargetRef) -> TargetRef:
        self.validate_lock_ref(target)
        ensured = self._call_plugin('ensure_root', target, self._context())
        return TargetRef.from_dict(ensured, resolved_revision=target.revision is not None)

    def resolve_revision(self, target: TargetRef) -> TargetRef:
        resolved = self._call_plugin('resolve_lock_ref', target, self._context())
        return TargetRef.from_dict(resolved, resolved_revision=True)

    def validate_config(self, target: TargetRef) -> None:
        self._call_plugin('validate_config', target, self._context())

    def validate_lock_ref(self, target: TargetRef) -> None:
        self._call_plugin('validate_lock_ref', target, self._context())

    def _context(self) -> TargetContext:
    ...
    def from_runtime(
        ...
        *,
        ...
        for key, record in runtime.capabilities.items():
            ...
            registry.register(
                key.id,
                _RuntimeTargetAdapter(record.capability, capability_id=key.id, owner=_runtime_owner(record.owner, record.capability), auth=auth),
                plugin_id=record.owner_id,
...
def _runtime_owner(owner: CapabilityOwner | str | None, capability: object) -> CapabilityOwner | None:
    version = getattr(capability, 'plugin_version', None)
    if owner is None:
        return None
