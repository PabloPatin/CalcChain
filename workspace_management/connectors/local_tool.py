...
class LocalTool:
    def load_file_from_config(self, src_path: str | Path, dst_dir: str = '', *,
        ...
        :param dst_dir: Путь к директории относительно корня рабочей области,
                             куда поместить файл
        :param file_name: Имя файла в рабочей области (если надо переименовать)
        :param kwargs: Прочие аргументы, которые могу быть в словаре с конфигурацией
        """
        file_name = file_name or split_link(path)[1]
        local_path = os.path.join(dst_dir, file_name)
