...
* `url`: Ссылка на репозиторий или папку в нём.
* `check_exists`: Флаг, указывающий, следует ли проверять возможность подключения по указанной ссылке. Если True, путь
...
* `rel_path`: Путь к папке в репозитории относительно SvnClient.url. Если не указан, информация будет получена для
  SvnClient.url.
